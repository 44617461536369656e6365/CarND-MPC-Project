...
// constants
double const c_ref_state[] = {
 5000., 2500., 1.
};
double const c_actuators[] = {
 1500., 1.
};
double const c_min_gap[] = {
 5000., 1.
};
...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars)
    {
        ...
        for (auto t = 0u; t < N; t++) {
            ...
            fg[0] += c_ref_state[2] * CppAD::pow(vars[v_start + t] - MPC::constants::ref_v, 2);
        }
        ...
        for (auto t = 1u; t < N; t++) {
            ...
            fg[1 + epsi_start + t] =
                epsi1 - ((psi0 - psides0) + v0 * delta0 / MPC::constants::Lf * dt);
        }
    }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
    ...
    auto const x = state[0];
    auto const y = state[1];
    auto const psi = state[2];
    auto const v = state[3];
    auto const cte = state[4];
    auto const epsi = state[5];
